...
internal data class HierarchyDiagnostic private constructor(private val root: Step) :
    ...
    private class Step(val name: String, val type: StepType, val parent: Step? = null) : DiagnosticSender {
        ...
        override fun createTask(name: String): DiagnosticSender = Step(name, TASK, this)

        override fun addStep(message: String, symbol: KSNode?) {
            messages.add(DiagnosticMessage(STEP, message.trim(), symbol))
        }

        override fun addWarning(message: String, symbol: KSNode?) {
            messages.add(DiagnosticMessage(WARNING, message.trim(), symbol))
        }

        override fun addError(message: String, symbol: KSNode?) {
            messages.add(DiagnosticMessage(ERROR, message.trim(), symbol))
        }
